...
function AdminPanel({ token, onLogout, onClose }) {
  const [tab, setTab] = useState("products") // "products" | "cashiers"
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onLogout} className="text-sm text-muted hover:text-red-500">Log out</button>
        </div>
      </header>
      ...
      <div className="bg-white border-b border-gray-200 px-6 flex gap-6">
        <button
          ...
          className={`py-3 text-sm font-medium border-b-2 ${tab === "products" ? "border-primary text-primary" : "border-transparent text-muted hover:text-ink"}`}
        >
          Products
        </button>
        <button
          onClick={() => setTab("cashiers")}
          className={`py-3 text-sm font-medium border-b-2 ${tab === "cashiers" ? "border-primary text-primary" : "border-transparent text-muted hover:text-ink"}`}
        >
          Cashiers
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {tab === "products" ? <ProductsTab token={token} /> : <CashiersTab token={token} />}
      </div>
    </div>
  )
}
            ...
            {products.map((p) => (
              <tr key={p.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap">
                  {editingId === p.id ? (
                    <button onClick={() => saveEdit(p.id)} className="text-primary text-xs font-medium mr-2">Save</button>
                  ) : (
                    <button onClick={() => startEdit(p)} className="text-primary text-xs font-medium mr-2">Edit</button>
                  )}
                  {p.is_active && (
                    <button onClick={() => handleDeactivate(p.id)} className="text-red-500 text-xs">Deactivate</button>
                  )}
                </td>
              </tr>
            ))}
...
function CashiersTab({ token }) {
  ...
  const loadCashiers = async () => {
    try {
      const data = await api.getAllCashiers(token)
      setCashiers(data)
    } catch (e) {
      setError(e.message)
    }
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await api.createCashier(newName, newPin, token)
      setNewName("")
      setNewPin("")
      setShowAddForm(false)
      loadCashiers()
  ...
  const handleDeactivate = async (cashierId) => {
    if (!confirm("Remove this cashier? They will no longer be able to log in to the till.")) return
    try {
      await api.deactivateCashier(cashierId, token)
      loadCashiers()
    } catch (e) {
      setError(e.message)
    }
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-muted">{cashiers.length} cashiers</p>
        <button
          onClick={() => setShowAddForm(!showAddForm)}
          className="bg-accent hover:bg-accent-dark text-white text-sm font-medium px-4 py-2 rounded-lg"
        >
      ...
      {showAddForm && (
        <form onSubmit={handleAdd} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 flex gap-3 items-end">
          ...
          <div className="w-32">
            <label className="text-xs text-muted block mb-1">PIN (4+ digits)</label>
            <input required type="text" inputMode="numeric" value={newPin}
              onChange={(e) => setNewPin(e.target.value.replace(/\D/g, ""))}
              className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm" />
          </div>
          <button type="submit" className="bg-primary hover:bg-primary-dark text-white text-sm font-medium px-4 py-2 rounded-lg">
            Save
          </button>
        </form>
      )}
          ...
          <tbody>
            {cashiers.map((c) => (
              <tr key={c.id} className="border-t border-gray-100">
                ...
                <td className="px-4 py-2 text-right">
                  <span className={c.is_active ? "text-success" : "text-muted"}>
                    {c.is_active ? "Active" : "Inactive"}
                  </span>
                </td>
                <td className="px-4 py-2 text-right">
                  {c.is_active && (
                    <button onClick={() => handleDeactivate(c.id)} className="text-red-500 text-xs">Remove</button>
                  )}
                </td>
              </tr>
            ))}
